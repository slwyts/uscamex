-- D1 (SQLite) port of Token/offchain storage used by the live Worker.
...
CREATE INDEX IF NOT EXISTS idx_chain_events_token_block
    ON chain_events (token_address, block_number, log_index);
...
CREATE INDEX IF NOT EXISTS idx_config_history_key ON protocol_config_history (token_address, key, id DESC);
...
CREATE INDEX IF NOT EXISTS idx_node_history_address ON node_history (token_address, node_address, id DESC);
